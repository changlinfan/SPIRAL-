function [UAVBSsSet, UAVBSsR, UEsPositionOfUAVServedBy] = ourAlgorithm_withDrawStep(locationOfUEs, maxDataTransferRateOfUAVBS, minDataTransferRateOfUEAcceptable, config)
    ...
    if size(uncoveredBoundaryUEsSet,1) > 1
        plot([uncoveredBoundaryUEsSet(:,1);uncoveredBoundaryUEsSet(1,1)], [uncoveredBoundaryUEsSet(:,2);uncoveredBoundaryUEsSet(1,2)], 'Color', boundaryColor, 'LineStyle', "--");
    end
    ...
    while ~isempty(uncoveredBoundaryUEsSet)
        ...
        % 涵蓋
        [newPositionOfUAVBS, newUEsSet, r] = ourLocalCover_withDrawStep(centerUE, centerUE, setdiff(uncoveredUEsSet, centerUE, 'rows'), locationOfUEs, isUEsCovered, maxNumOfUE, config);
